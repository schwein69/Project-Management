...
Il \textit{Project Scoping Meeting} costituisce la prima occasione d'incontro ufficiale tra il fornitore (TechSolution) e il committente (NexusLog S.p.A.). Applicando le nostre politiche aziendali, si è svolta una prima sessione seguendo l'agenda presentata nella sezione precedente. Tale documento è stato distribuito a tutte le figure aziendali presenti alla riunione e prevede inoltre una sezione alle note e alla pianificazione del successivo. 

Nella costruzione dell'agenda si sono effettuate le seguenti considerazioni strategiche:
\begin{itemize}
    \item Non disponendo al momento di un ruolo esplicito di Facilitatore all'interno della \textit{startup}, tale incarico è ricoperto tutti i membri del team, con l'obiettivo di garantire la massima partecipazione e il rispetto dei tempi. In particolare, il membro dello staff con maggiore esperienza in ambito \textit{Project Management} si occupa di moderare la discussione e di sintetizzare le decisioni prese.
    \item I tempi relativi alla durata di ogni argomento in agenda sono stati stimati a partire dal contenuto della prima mail di contatto pervenuta da NexusLog, mettendo comunque in conto un'alta probabilità di discostamento a causa dell'incertezza intrinseca che caratterizza un dominio innovativo come quello dei droni.
\end{itemize}

Dal momento che la discussione delle \textit{Conditions of Satisfaction} (CoS) non si esaurisce in un unico punto dell'ordine del giorno, si è scelto di prevedere un'apposita area all'interno del \textit{template} in cui annotare man mano gli obiettivi e i vincoli del progetto. Non essendoci esperti in materia, TechSolution ha maturato una propria opinione in merito, coerente con l'approccio \textit{Scrumban}: i CoS rappresentano le condizioni inalienabili alla base del progetto (es. il rispetto del budget per il PoC o la tutela della \textit{brand reputation}). Essi non coincidono strettamente con i \textit{Goal}, gli \textit{Objective} o i \textit{Success Criteria}, ma contribuiscono a renderli chiari. Alcune di queste condizioni si riferiscono a obiettivi operativi, altre ai \textit{requirement} strategici aziendali.

Dato che il cliente può fornire materiale di varia natura (es. specifiche hardware dei droni e la documentazione tecnica), si predispone anche un'apposita sezione in cui tener traccia dei riferimenti a quanto fornito. La compilazione digitale del documento è stata scelta per diverse ragioni:
\begin{enumerate}
    \item facilita un eventuale processamento automatico e la ricerca delle informazioni in esso contenute;
    \item velocizza il processo di annotazione (considerando come il membro dello staff incaricato sia frequentemente impegnato anche nel dirigire la discussione);
    \item rende il contenuto del documento immediatamente fruibile in \textit{cloud} da parte di tutti gli \textit{stakeholder}.
\end{enumerate}

Un esempio di documento compilato per quanto concerne la prima sessione è disponibile nel seguente allegato, a questo link: \href{https://raw.githubusercontent.com/schwein69/Project-Management/master/docs/ScopeMeeting/Session1/scope_meeting.pdf}{scope\_meeting.pdf}.


A causa dell'estrema complessità del progetto \textit{Shipping-on-the-Air}, durante il primo incontro è emersa fin da subito la necessità di frammentare l'analisi e pianificare altri \textit{meeting}. Riconoscendo la presenza di molteplici sottosistemi critici, si è preferito limitare la prima riunione a un inquadramento generale del problema, organizzando sessioni verticali per entrare nei dettagli di ciascun modulo. Tenendo conto della disponibilità manifestata dall'Ing. Costa a sostenere più incontri ravvicinati, l'elenco delle successive sessioni di \textit{scoping} è di seguito riportato:
\begin{itemize}
    \item \textbf{04/05/2026} -- Agenda dedicata al sottosistema Gestione Ordini e Controllo Vincoli di Flotta (autonomia batteria e \textit{payload}).
    \item \textbf{06/05/2026} -- Agenda dedicata al sottosistema di Navigazione (\textit{Routing \& GPS}) e agli algoritmi di aggiramento degli ostacoli.
    \item \textbf{08/05/2026} -- Agenda dedicata ai sottosistemi di Tracciamento Live (telemetria satellitare) e Notifiche/Pagamenti.
\end{itemize}



Nell'intervallo di tempo compreso tra una sessione e la successiva, TechSolution si impegna anche alla realizzazione di \textit{mockup} (sottoposti al giudizio del committente tramite email) relativi alle interfacce grafiche, sia per quanto riguarda la \textit{dashboard} web per gli operatori logistici, sia per l'applicazione mobile rivolta ai clienti.

Dopo aver discusso nel dettaglio ogni sottosistema (comprendendo problematiche, \textit{desiderata} e modalità di raccolta requisiti), si è infine pianificato un ultimo \textit{meeting} (successivo alla classificazione del progetto e all'identificazione dei rischi) per la scelta del \textit{PMLC Model} più idoneo a colmare il \textit{gap} tecnologico individuato. In questa occasione si è anche effettuata la stesura e l'approvazione della bozza del POS (\textit{Project Overview Statement}):
\begin{itemize}
    \item \textbf{11/05/2026} -- Agenda del \textit{meeting} conclusivo per la definizione finale dello \textit{scope}.
\end{itemize}

Per quanto concerne il budget (limitato in questa fase a un \textit{Proof of Concept}) e i tempi di consegna, si è prevista una discussione iniziale di questi aspetti — condotta dal \textit{Project Manager} del cliente — già all'interno della prima sessione (in modo tale da non considerare nemmeno la pianificazione delle riunioni successive in assenza della copertura finanziaria per farlo). Si osserva inoltre come un'ulteriore validazione di questi temi sia stata effettuata all'interno dell'ultima sessione dal \textit{Project Manager} di TechSolution (ora consapevole di tutti i dettagli tecnici e in grado di fare una stima reale).

Si sottolinea infine come, durante l'ultimo incontro, Mario Rossi abbia manifestato la volontà di gestire interamente \textit{in-house} la formazione degli operatori di NexusLog. Ha richiesto pertanto a TechSolution di formare esclusivamente lui e il suo \textit{team} tecnico diretto, un processo agevolato dal suo continuo coinvolgimento e dalla sua solida esperienza pregressa in ambito IT e \textit{Digital Transformation}.

Unendo tutte le informazioni raccolte sui bisogni del cliente, il documento descrittivo dell'intero progetto è stato così consolidato e formalizzato.
